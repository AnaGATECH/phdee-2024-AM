...
*** Question 2. ***  I neeed to edit this part
*******************************************************************************
...
* Event study
	...
	twoway rarea low high order if order<=29 & order >= -29 , fcol(gs14) lcol(white) msize(1) /// estimates
		...
		|| scatteri 0 -29 0 29, recast(line) lcol(gs8) lp(longdash) lwidth(0.5) /// zero line 
			...
			ylab(, nogrid labs(3)) ///
			legend(off) ///
			xtitle("Days to treatment", size(5)) ///
...
*** Question 3. ***
*******************************************************************************
eventdd energy temperature precipitation relativehumidity, hdfe absorb(id) timevar(event_time) cluster(id) graph_op(ytitle("Daily energy consumption (kWh)", size(5)) xlabel(-30(10)30) xtitle("Days to treatment", size(5)))
...
*******************************************************************************
	...
	estat simple
	estat event
	csdid_plot, title("Event Study") ytitle("Daily energy consumption (kWh)", size(3)) xlabel(-30(10)30) xtitle("Days to treatment", size(3)) xline(-.5, lpattern(dash) lcolor(purple) lwidth(0.7))
